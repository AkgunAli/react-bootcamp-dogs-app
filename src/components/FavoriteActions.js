...
const FavoriteActions = (props) => {
    ...
    return (
        <div>
            ...
            {foundDog ?
                <Button color="danger" disabled={props.id === props.loadingId} onClick={() => toggle(props.id)}>Favorilerden Çıkar</Button>
                : <Button color="success" disabled={props.id === props.loadingId} onClick={() =>  toggle(props.id)}>Favoriye Ekle</Button>
            }
        </div>
    );
};
